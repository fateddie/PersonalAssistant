...
@router.get("/events")
def list_events(
    calendar_id: str = "primary", max_results: int = 10, time_min: str = None, time_max: str = None
):
...
def hide_calendar_event(google_event_id: str):
    ...
    try:
        ...
        with engine.begin() as conn:
            conn.execute(
                text(
                    "INSERT OR IGNORE INTO hidden_calendar_events (google_event_id, event_title) VALUES (:event_id, :title)"
                ),
...
def unhide_calendar_event(google_event_id: str):
    ...
    try:
        with engine.begin() as conn:
            conn.execute(
                text("DELETE FROM hidden_calendar_events WHERE google_event_id = :event_id"),
                {"event_id": google_event_id},
            )
...
def list_hidden_events():
    ...
    try:
        with engine.connect() as conn:
            result = conn.execute(
                text(
                    "SELECT google_event_id, event_title, hidden_at FROM hidden_calendar_events ORDER BY hidden_at DESC"
                )
            )
            hidden_events = [
                {"google_event_id": row[0], "title": row[1], "hidden_at": row[2]} for row in result
            ]
