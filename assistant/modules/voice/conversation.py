...
def process_chat_message(message: str, conversation_history: list = None) -> dict:
        ...
        if stage == "ask_calendar":
            ...
            if message_lower in ["yes", "y", "yeah", "yep", "sure"]:
                ...
                response["response_text"] = (
                    "What days and time? (e.g., 'weekdays 7:30-9am' or 'mon wed fri 9-10am')"
                )
                return response
                ...
                response["action"] = "goal_flow_continue"
                response["response_text"] = (
                    "Ok, no calendar blocking. What should I call this goal?"
                )
                return response
            else:
                response["action"] = "goal_flow_continue"
                response["response_text"] = (
                    "Would you like to block time in your calendar? (yes/no)"
                )
                return response
            ...
            if days and start_time:
                ...
                response["response_text"] = (
                    f"Got it - {days_str} from {format_time(start_time)} to {format_time(end_time)}. What should I call this goal?"
                )
                return response
            else:
                response["action"] = "goal_flow_continue"
                response["response_text"] = (
                    "I didn't catch that. Please specify days and time (e.g., 'weekdays except wednesday 7:30-9am')"
                )
                return response
            ...
            response["action"] = "goal_flow_continue"
            response["response_text"] = (
                "What category is this goal? (educational, fitness, work, personal, creative, other)"
            )
            return response
            ...
            if pending_goal.get("days"):
                ...
                response["response_text"] = (
                    f"I'll create goal **'{title}'** ({category}) with sessions on **{days_str}** from **{time_str}** and add them to your Google Calendar. Confirm?"
                )
    ...
    if st.session_state.get("awaiting_info"):
        ...
        if awaiting_type == "goal_name":
            ...
            response["response_text"] = (
                f"I'll create goal **'{goal_name.title()}'**. Should I go ahead?"
            )
            return response
            ...
            response["response_text"] = (
                f"I'll create task **'{task_name.title()}'** for today. Should I go ahead?"
            )
            return response
    ...
    if details["action"] == "create":
        if details["type"] == "goal" and details["title"]:
            ...
            if details["days"] and details["start_time"]:
                ...
                time_str = (
                    f"{format_time(details['start_time'])} to {format_time(details['end_time'])}"
                )
                response["response_text"] = (
                    f"I'll create goal **'{details['title'].title()}'** with sessions on **{days_str}** from **{time_str}**. Should I go ahead?"
                )
            else:
                ...
                response["response_text"] = (
                    f"I'll create goal **'{details['title'].title()}'**. Should I go ahead?"
                )
            return response
            ...
            response["response_text"] = (
                f"I'll create task **'{details['title'].title()}'** for today. Should I go ahead?"
            )
            return response
        ...
        if details["type"] == "goal":
            ...
            response["response_text"] = (
                "Great! Would you like to block out time in your calendar for this goal?"
            )
            ...
            response["response_text"] = (
                f"I'd like to create a {details['type']} for you. What would you like to call it?"
            )
            response["action"] = "need_more_info"
            return response
        else:
            response["response_text"] = (
                "What would you like me to create? (goal, task, session, meeting)"
            )
            response["action"] = "need_more_info"
            return response
