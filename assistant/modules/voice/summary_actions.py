...
def get_daily_summary(client) -> str:
    ...
    try:
        ...
        type_emoji = {'goal': '🎯', 'task': '✅', 'session': '⏰', 'meeting': '👥', 'webinar': '📺', 'appointment': '📅'}
...
def check_and_summarize_emails() -> str:
    ...
    try:
        ...
        if response.status_code != 200:
            return "❌ Couldn't check emails. Make sure the orchestrator is running at localhost:8000"
        ...
        summary_parts.append(f"**{total} emails** - {by_priority.get('HIGH', 0)} high, {by_priority.get('MEDIUM', 0)} medium, {by_priority.get('LOW', 0)} low priority\n")
        ...
        for email in emails[:5]:
            priority_icon = "🔴" if email.get('priority') == 'HIGH' else "🟡" if email.get('priority') == 'MEDIUM' else "🟢"
...
def get_all_daily_tasks(client) -> str:
    ...
    try:
        ...
        upcoming_result = client.list_items(date_from=today, date_to=next_week, status='upcoming', limit=50)
